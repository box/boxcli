...
describe('Webhooks', () => {
	...
	describe('webhooks:get', () => {
		...
		test
			...
			.it('should get information about a webhook (JSON Output)', ctx => {
		...
		test
			...
			.it('should get information about a webhook (YAML Output)', ctx => {
	...
	describe('webhooks:delete', () => {
		...
		test
			...
			.it('should delete a webhook', ctx => {
				assert.equal(ctx.stderr, `Deleted webhook ${webhookId}${os.EOL}`);
			});
	});
	...
	describe('webhooks:create', () => {
		...
		test
			.nock(TEST_API_ROOT, api => api
				.post('/2.0/webhooks', expectedBody)
				.reply(200, fixture)
			)
			.stdout()
			.command([
	...
	describe('webhooks:update', () => {
		...
		test
			...
			.it('should update a webhook with triggers flag passed (JSON Output)', ctx => {
				assert.equal(ctx.stdout, fixture);
			});
		...
		test
			...
			.it('should update a webhook with triggers flag passed (YAML Output)', ctx => {
				assert.equal(ctx.stdout, yamlOutput);
			});
