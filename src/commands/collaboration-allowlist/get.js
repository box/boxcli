...
class CollaborationAllowlistGetCommand extends BoxCommand {
	async run() {
		...
		let allowlistEntry = await this.client.collaborationAllowlist.getAllowlistedDomain(args.id, options);
		await this.output(allowlistEntry);
	}
}

CollaborationAllowlistGetCommand.description = 'Get a collaboration allowlist entry';
CollaborationAllowlistGetCommand.examples = ['box collaboration-allowlist:get 12345'];
CollaborationAllowlistGetCommand._endpoint = 'get_collaboration_whitelist_entries_id';
